...
router.put(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    try {
      const item = await prisma.actaItem.findUnique({ where: { id: itemId } })
      ...
      const updatedItem = await prisma.actaItem.update({
        where: { id: itemId },
    ...
    try {
      const { item: updatedItem, acta: updatedActa } = await prisma.$transaction(async (tx) => {
        ...
        if (item.categoria === 'droga') {
          ...
          if (!existing) {
            throw new Error('DepositoProducto no encontrado en inventario de estuche')
          }
          ...
          if (!existing) {
            throw new Error('DepositoProducto no encontrado en inventario de etiqueta')
          }
