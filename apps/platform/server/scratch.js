...
function applyCommonHeader(ws, title, cliente, year, month) {
  ws.mergeCells('A1:F1');
...
function applyResumenMetrics(ws, reporte) {
  ws.mergeCells('A9:F9');
  ...
  metrics.forEach(m => {
    ...
    for (let r = 10; r <= 12; r++) {
      for (let c = cols[0]; c <= cols[1]; c++) {
        ...
        cell.border = { top: { style: 'thin', color: { arg: COLORS.border } }, left: { style: 'thin', color: { arg: COLORS.border } }, bottom: { style: 'thin', color: { arg: COLORS.border } }, right: { style: 'thin', color: { arg: COLORS.border } } };
      }
    }
  });
}

function buildMensualSheet(wb, cliente, reporte) {
  const ws = wb.addWorksheet('Ventas', { properties: { tabColor: { arg: COLORS.primary } } });
  setupPrintOptions(ws);
  ws.columns = [ { width: 38 }, { width: 22 }, { width: 11 }, { width: 11 }, { width: 11 }, { width: 14 } ];
  applyCommonHeader(ws, 'REPORTE DE VENTAS POR CLIENTE', cliente, reporte.year, reporte.month);
...
function buildAnualSheets(wb, cliente, reporte) {
  const wsRes = wb.addWorksheet('Resumen anual', { properties: { tabColor: { arg: COLORS.primary } } });
  setupPrintOptions(wsRes);
  wsRes.columns = [ { width: 20 }, { width: 22 }, { width: 22 }, { width: 22 }, { width: 10 }, { width: 10 } ];
  ...
  wsProd.columns = [ { width: 38 }, { width: 22 }, { width: 11 }, { width: 11 }, { width: 11 }, { width: 14 } ];
...
async function main() {
  const cliente = { nombre: 'DEMO Agropecuaria Norte', cuit: '20-22222222-2' };
  
  const reporteMensual = {
    modo: 'mensual', year: 2026, month: 8,
    pedidosDespachados: 1, productosDistintos: 2, unidadesTotales: 35,
    productos: [
      { nombre: 'Producto A', sku: 'SKU-001', unidadesPorCaja: 10, cajas: 2, sueltos: 5, unidades: 25 },
      { nombre: 'Producto B', sku: 'SKU-002', unidadesPorCaja: 5, cajas: 2, sueltos: 0, unidades: 10 },
    ]
  };
  const wb1 = new ExcelJS.Workbook();
  buildMensualSheet(wb1, cliente, reporteMensual);
  await wb1.xlsx.writeFile('C:/Users/Deposito/.gemini/antigravity-cli/brain/b73237b5-06ac-4602-8ee6-1a6c80acfb3a/scratch/ventas-demo-agropecuaria-norte-2026-08.xlsx');
  
  const reporteAnual = {
    modo: 'anual', year: 2026,
    pedidosDespachados: 5, productosDistintos: 2, unidadesTotales: 150,
    meses: [
      { month: 1, pedidosDespachados: 2, productosDistintos: 2, unidadesTotales: 50 },
      { month: 8, pedidosDespachados: 3, productosDistintos: 2, unidadesTotales: 100 },
    ],
    productos: [
      { nombre: 'Producto A', sku: 'SKU-001', unidadesPorCaja: 10, cajas: 10, sueltos: 5, unidades: 105 },
      { nombre: 'Producto B', sku: 'SKU-002', unidadesPorCaja: 5, cajas: 9, sueltos: 0, unidades: 45 },
    ]
  };
  const wb2 = new ExcelJS.Workbook();
  buildAnualSheets(wb2, cliente, reporteAnual);
  await wb2.xlsx.writeFile('C:/Users/Deposito/.gemini/antigravity-cli/brain/b73237b5-06ac-4602-8ee6-1a6c80acfb3a/scratch/ventas-demo-agropecuaria-norte-2026.xlsx');
