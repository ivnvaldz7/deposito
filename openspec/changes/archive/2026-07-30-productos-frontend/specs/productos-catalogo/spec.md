...
MUST be encargado-only. Fields conditional by category: etiqueta/estuche require presentation + ≥1 market; frasco requires presentation (no markets); droga/MP requires neither. Creates directly as ACTIVO via `POST /api/deposito/productos`.

| Scenario | GIVEN | WHEN | THEN |
|----------|-------|------|------|
| Create etiqueta | encargado, form filled | category etiqueta + 2 markets submitted | POST succeeds, product is ACTIVO |
...
MUST allow encargado to edit. PENDIENTE_REVISION: code + name + presentation + markets editable. ACTIVO/INACTIVO: name + presentation only (audited).
